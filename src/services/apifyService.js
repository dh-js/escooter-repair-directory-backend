...
export const crawlerGooglePlaces = async (
  ...
  try {
    ...
    const run = await apifyClient.actor("compass/crawler-google-places").call({
      ...
      scrapeDirectories: false, // true = include places inside other places (e.g., stores in malls)
      deeperCityScrape: true, // true = more thorough search in populated areas (slower but more results)

      // Review Settings
      maxReviews: 1, // 0 = no reviews, 99999 = all reviews. Max 5000 per place item
      ...
      // Questions
      maxQuestions: 0, // 0 = first Q&A only, 999 = all questions
